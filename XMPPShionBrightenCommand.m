...
#import "XMPPShionBrightenCommand.h"
#import "AppDelegate.h"
#import "Shion.h"
#import "DeviceDictionary.h"


@implementation XMPPShionBrightenCommand
...
//	NSArray * devices = [delegate systemPropertyForName:@"devices"];
	
	NSString * status = @"executing";
	
	if (deviceId != nil)
	{
		NSString * title = @"No matching devices found for brightening.";
		
		NSEnumerator * deviceIter = nil; // [devices objectEnumerator];
		...
		while (device = [deviceIter nextObject])
		{
			...
			if ([deviceId isEqual:deviceString])
			{
				...
				title = [NSString stringWithFormat:@"%@ brightened.", [device valueForKey:SHION_NAME]];
	...
	{	
		...
		[title setStringValue:@"Brighten Device"];
		[x addChild:title];
		
		NSXMLElement * instructions = [NSXMLElement elementWithName:@"instructions"];
		[instructions setStringValue:@"Please select a device from list below to brighten."];
		...
		while (device = [deviceIter nextObject])
		{
			if ([[device valueForKey:SHION_TYPE] isEqual:CONTINUOUS_DEVICE])
			{
	...
	[command setAttributesAsDictionary:[NSDictionary dictionaryWithObjectsAndKeys:session, @"sessionid", @"brighten", @"node", status, @"status", nil]];
